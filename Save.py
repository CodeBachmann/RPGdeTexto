...
class Salvar:
    def __init__(self):
        file = '\\ArquivoSave\\save.txt'
        self.status = str(os.getcwd()+file)
        file = '\\ArquivoSave\\saveHabilidades.txt'
        self.habilidades = str(os.getcwd()+file)
        file = '\\ArquivoSave\\saveArtefatos.txt'
        self.artefatos = str(os.getcwd()+file)

    def deletarSave(self, jogador, salvado):
        save = open(self.status,'r')
        listaNumeros = save.read().splitlines()
        save.close()
        save = open(self.habilidades,'r')
        listaHabilidades = save.read().splitlines()
        save.close()
        save = open(self.artefatos,'r')
        ...
        textoNovo = textoNovo.replace("[", "")
        textoNovo = textoNovo.replace("]", "")
        save = open(self.artefatos,'w+')
        save.write(textoNovo)
        save.close()

        
    def escolherSave(self, jogador):
        save = open(self.status,'r')
        listaNumeros = save.read().splitlines()
        save.close()
        cont = 1
    ...
    def salvar (self, jogador, salvado):
    ...
    def carregar(self, jogador, salvado):
